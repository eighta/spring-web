...
<form:form id="diningForm" method="post" modelAttribute="diningForm">
	<fieldset> 
		...
		<ul>
			...
			<!-- TODO-08: We need one more element for a one-click reward check-box.
			              The field you need already is in DiningForm and the labels
			              and error-codes are already in messages.properties. -->
		</ul>
		
		<button id="rewardButton" name="_eventId_<c:out value="${rewardButtonEvent}" default="reward"/>" type="submit">
			<fmt:message key="command.reward" var="defaultRewardButtonText" />
			<c:out value="${rewardButtonText}" default="${defaultRewardButtonText}" />
		</button>
